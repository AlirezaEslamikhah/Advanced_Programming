...
namespace C11.Test
{
    ...
    public class UnitTest1
    {
        ...
        public void TestCircle()
        {
            ...
            C1.Move(3,1);
            Assert.AreEqual(C1.Pc.x,6);
            Assert.AreEqual(C1.Pc.y,9);
        }
        [TestMethod]
        public void TestSquare()
        {
            Square S1 = new Square("s1",new Point(0,0),2);
            Assert.AreEqual(S1.p4.x , 0);
            Assert.AreEqual(S1.p2.x , 2);
            Assert.AreEqual(S1.p3.x , 2);
            Assert.AreEqual(S1.p4.y , 2);
            Assert.AreEqual(S1.p2.y , 0);
            Assert.AreEqual(S1.p3.y , 2);
            S1.Move(0,1);
            Assert.AreEqual(S1.p4.x , 0);
            Assert.AreEqual(S1.p2.x , 2);
            Assert.AreEqual(S1.p3.x , 2);
        ...
        public void BoardAdd_Move(){
            ...
            B1.AddShape(C1);
            B1.AddShape(S1);
            Assert.AreEqual(B1.total.Count , 3);
            Assert.AreEqual(B1.total[1] , C1);
